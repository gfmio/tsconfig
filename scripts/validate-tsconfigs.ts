...
function validateTsconfigFile(filePath: string): ValidationResult {
	...
	try {
		...
		let configToValidate = configFile.config;
		if (isDenoConfig && configFile.config.compilerOptions?.lib) {
			// Filter out Deno-specific libs that TypeScript doesn't recognize
			const denoSpecificLibs = ["Deno.NS", "deno.ns", "deno.window", "deno.worker"];
			configToValidate = {
				...configFile.config,
				compilerOptions: {
					...configFile.config.compilerOptions,
					lib: configFile.config.compilerOptions.lib.filter(
						(lib: string) => !denoSpecificLibs.includes(lib),
					),
				},
			};
		}
...
async function main(): Promise<void> {
	...
	// Print detailed error report if there are any errors
	if (hasErrors) {
